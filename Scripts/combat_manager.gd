...
var SelectedUnit = DeployedUnits.instantiate() as Unit
...
func PlaceUnit(TileCoords : Vector3i, TileNormal: Vector3i) -> bool:
	if $"../BoardManager".GetTileType(TileCoords + TileNormal) is TileDictionary.PlayerSpawner :
		SelectedUnit.position = $"../GridMap".map_to_local(TileCoords + TileNormal)
		$"../BoardManager".SetTile(TileCoords + TileNormal, TileDictionary.TestUnit.new())
